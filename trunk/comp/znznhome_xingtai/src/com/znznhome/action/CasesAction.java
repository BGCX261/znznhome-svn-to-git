...
@Component("casesAction")
@Scope("prototype")
public class CasesAction extends ActionSupport {
	private static final long serialVersionUID = 5492477698601698073L;
	...
	public String list() {
		if (pageNo != null && !pageNo.trim().equals("")) {
			...
			if(countNumber == 0) countNumber = 6;
			categoryid = categoryManager.getIdByEname(ename); 
			if(categoryid < 0) return ERROR;
			//直接点击信息维护时显示的页面；
			sql_count = "select count(*) from znzn_cases where recflag=1 and categoryid=" + categoryid;
			sql = "select * from znzn_cases where recflag=1 and categoryid=" + categoryid + " order by createtime desc";
			Class c = new Cases().getClass();
	...
	public String manager() {
		...
		if(action_search != null && action_search.equals("action_search")) {
			//通过查询进入
			sql_count = "select count(*) from znzn_cases where 1=1 ";
			sql = "select * from znzn_cases where 1=1 ";
			Class c = new Cases().getClass();
			...
			//直接点击信息维护时显示的页面；
			sql_count = "select count(*) from znzn_cases ";
			sql = "select * from znzn_cases order by createtime desc";
			Class c = new Cases().getClass();
