...
<ul class="daohang">
  ...
  <li class="bg2 ts2"><a href="product-list?ename=product01" target="_self">产品中心</a></li>
...
<div class="clearfloat"></div>
<div class="content">
  <div class="leftcon"><div style=" margin:0 auto; width:159px;"><img style="padding-top:16px;" src="images/con2_4.jpg"  width="159" height="44"/></div>
    <ul class="ldhbf">
<%
	String claz01 = "", claz02 = "", claz03 = "", claz04 = "", claz05 = "", claz06 = "";
	if(ename.equals("product01")) {
		claz01 = " class=\"fsxg\" ";
	}else if(ename.equals("product02")) {
		claz02 = " class=\"fsxg\" ";
	}else if(ename.equals("product03")) {
		claz03 = " class=\"fsxg\" ";
	}else if(ename.equals("product04")) {
		claz04 = " class=\"fsxg\" ";
	}else if(ename.equals("product05")) {
		claz05 = " class=\"fsxg\" ";
	}else if(ename.equals("product06")) {
		claz06 = " class=\"fsxg\" ";
	}
%>
      <li><a <%=claz01 %> href="product-list?ename=product01" target="_self">德国Becker（贝克）真空泵 </a></li>
      <li><a <%=claz02 %> href="product-list?ename=product02" target="_self">德国GardnerDenver（格南登福里其乐）真空泵</a></li>
      <li><a <%=claz03 %> href="product-list?ename=product03" target="_self">德国Sterling-sihi（希赫）水环真空泵</a></li>
      <li><a <%=claz04 %> href="product-list?ename=product04" target="_self">德国Busch（普旭）真空泵 </a></li>
      <li><a <%=claz05 %> href="product-list?ename=product05" target="_self">法国leybold（莱宝）真空泵</a></li>
      <li><a <%=claz06 %> href="product-list?ename=product06" target="_self">美国 Quincy（昆西）螺杆真空泵</a></li>
    </ul>
  </div>
  <div class="rightcon"> <div class="lxwbt2">产品列表</div>
    <ul class="ultp">
 	...
 	if(list_product != null) {
	    for(Iterator<Product> it = list_product.iterator(); it.hasNext();) {
	    	Product product = it.next();
	  		...
	  		String content = CommonUtil.filterHtmlAndSubStr(product.getContent(), 130);
			String createtime = CommonUtil.getTime(product.getCreatetime());
%>

      <li> 
      <a href="product.jsp?id=<%=id %>" target="_blank"><img src="upload/<%=picurl %>" /></a> 
      <a class="txt" href="product.jsp?id=<%=id %>" target="_blank"><%=title %></a> 
      </li>
...
%>
	  ...
	  <% } else { %>
		  ...
		  <a href="product-list?ename=<%=ename%>&pageNo=<%=pageNo-1%>"> <  Prev</a>
	  ...
	  <%    }else{ %>
		  ...
		  <a href="product-list?ename=<%=ename%>&pageNo=<%=i%>" target="_self"><%=i %></a>
	  ...
	  <%}else {%>
		  ...
		  <a href="product-list?ename=<%=ename%>&pageNo=<%=pageNo+1%>"> Next  > </a>
...
<map name="Map" id="Map">
  <area shape="circle" coords="21,20,17" onclick="javascript:window.open('http://b.qq.com/webc.htm?new=0&sid=359494582&o=星泰&q=7', '_blank', 'height=502, width=644,toolbar=no,scrollbars=no,menubar=no,status=no');" />
  <area shape="circle" coords="60,19,16" href="feedBack-list" /><area shape="circle" coords="101,20,18" href="contact.jsp" />
