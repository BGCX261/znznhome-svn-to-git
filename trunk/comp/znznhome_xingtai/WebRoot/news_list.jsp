...
<ul class="daohang">
  ...
  <li class="bg2 ts2"><a href="news-list?ename=news01" target="_self">新闻中心</a></li>
...
<div class="clearfloat"></div>
<div class="content">
  <div class="leftcon"><div style=" margin:0 auto; width:159px;"><img style="padding-top:16px;" src="images/con2_2.jpg"  width="159" height="44"/></div>
    <ul class="ldhbf">
<%
	if(ename.equals("news01")) {
%>
      <li><a class="fsxg" href="news-list?ename=news01" target="_self">公司新闻</a></li>
	  <li><a href="news-list?ename=news02" target="_self">行业新闻 </a></li>
<%		
	} else {
%>
	      <li><a href="news-list?ename=news01" target="_self">公司新闻</a></li>
		  <li><a class="fsxg" href="news-list?ename=news02" target="_self">行业新闻 </a></li>
<%			
	}
%>    
    </ul>
  </div>
  <div class="rightcon"><div class="lxwbt2">新闻列表</div>  
  
  <div class="liebiaocon">
<ul class="ul1">
 	...
 	if(list_news != null) {
	    for(Iterator<News> it = list_news.iterator(); it.hasNext();) {
	    	News news = it.next();
	  		...
	  		String picurl = CommonUtil.setDefaultPic(news.getPicurl(),"temp.jpg");
	  		String content = CommonUtil.filterHtmlAndSubStr(news.getContent(), 130);
			String createtime = CommonUtil.getTime(news.getCreatetime());
%>

        <li>
          <div class="lbcon"><a href="upload/<%=picurl %>" target="_blank"><img width="113px" height="100px" src="upload/<%=picurl %>" /></a></div>
          <div class="rbcon">
            <div class="bt"><span class="spanleft"><a href="news.jsp?id=<%=id %>" target="_blank"><%=title %></a></span><span class="spanright">[<%=createtime %>]</span></div>
            <p> <%=content %><a class="more" title="" href="news.jsp?id=<%=id %>" target="_blank">详细信息</a></p>
          </div>
        </li>
...
%>    
	  ...
	  <% } else { %>
		  ...
		  <a href="news-list?ename=<%=ename%>&pageNo=<%=pageNo-1%>"> <  Prev</a>
	  ...
	  <%    }else{ %>
		  ...
		  <a href="news-list?ename=<%=ename%>&pageNo=<%=i%>" target="_self"><%=i %></a>
	  ...
	  <%}else {%>
		  ...
		  <a href="news-list?ename=<%=ename%>&pageNo=<%=pageNo+1%>"> Next  > </a>
