...
public class ProductAction extends ActionSupport {
	...
	public String list() {
		if (pageNo != null && !pageNo.trim().equals("")) {
			...
			if(countNumber == 0) countNumber = 6;
			//直接点击信息维护时显示的页面；
			sql_count = "select count(*) from product where recflag=1 and categoryid=" + categoryid;
			sql = "select * from product where recflag=1 and categoryid=" + categoryid + " order by createtime desc";
			Class c = new Product().getClass();
	...
	public String manager() {
		...
		if(action_search != null && action_search.equals("action_search")) {
			//通过查询进入
			sql_count = "select count(*) from product where 1=1 ";
			...
			sql = "select * from product order by createtime desc";
			Class c = new Product().getClass();
