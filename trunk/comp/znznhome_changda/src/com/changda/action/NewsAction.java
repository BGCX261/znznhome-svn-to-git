...
public class NewsAction extends ActionSupport {
	...
	public String list() {
		if (pageNo != null && !pageNo.trim().equals("")) {
			...
			if(countNumber == 0) countNumber = 6;
			//直接点击信息维护时显示的页面；
			sql_count = "select count(*) from news where recflag=1 and categoryid=" + categoryid;
			sql = "select * from news where recflag=1 and categoryid=" + categoryid + " order by createtime desc";
			Class c = new News().getClass();
	...
	public String manager() {
		...
		if(action_search != null && action_search.equals("action_search")) {
			//通过查询进入
			sql_count = "select count(*) from news where 1=1 ";
