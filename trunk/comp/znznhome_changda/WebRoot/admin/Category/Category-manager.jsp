...
<link rel="stylesheet" type="text/css" href="css/admin/table-list.css">
  ...
  //int startNo = category_page.getStartNo();
  int pageSize = category_page.getPageSize();
  //int totalRecords = category_page.getTotalRecords();
  int pageNo = category_page.getPageNo();
  int totalPages = category_page.getTotalPages();
  %>
  <form action="admin/Category-manager.action" method="post" name="hshproductmgsearch">
...
List<Category> list = category_page.getList();
   	...
   	if(list != null) {
  	    for(Iterator<Category> it = list.iterator(); it.hasNext();) {
	  		...
	  		String templatefilename = category.getTemplatefilename();
...
%>
  ...
  <tr class="tr2">
	    ...
	    <td><a href="admin/Category-delete.action?categoryID=<%=id %>"><img style="border-width: 0px; " src="<%=basePath %>/images/admin/del.gif"/></a></td>
	    <td></td>
	  </tr>
	  	<%} //-- for循环 --
		...
		<!-- ------------以下HTML分页区------------ -->
			<form name="form1"  action="admin/Category-manager.action">			
			&nbsp;&nbsp;&nbsp;&nbsp; <a href="admin/Category-manager.action?pageNo=1">首页</a> 
			<a href="admin/Category-manager.action?pageNo=<%=pageNo-1%>">上一页</a> 
			<a href="admin/Category-manager.action?pageNo=<%=pageNo+1%>">下一页</a>   
			<a href="admin/Category-manager.action?pageNo=<%=totalPages%>">尾页</a> 
