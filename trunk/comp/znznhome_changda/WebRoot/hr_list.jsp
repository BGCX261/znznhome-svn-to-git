...
<body>
  ...
  List<Job> list_job = job_page.getList();
  %>
...
<div class="topht">
  ...
  <div class="topright"><span><a href="index.jsp" target="_self">主页</a><a href="#" target="_self">合作伙伴</a><a style="margin-right:0px;" href="contact.jsp" target="_self">联系我们</a></span>
    ...
    document.getElementById("daohang1").getElementsByTagName("a")[5].className="bg2 fsxg";
    </script>
...
<div class="con2">
<div class="leftcon l">
 <div class="title">您当前的位置：<a href="index.jsp" target="_self">首页</a> > <a class="ra" href="Job-list.action?countNumber=9" target="_self"> 招聘中心</a></div>
  ...
  <%
 	if(list_job != null) {
	    for(Iterator<Job> it = list_job.iterator(); it.hasNext();) {
	    	Job job = it.next();
	  		int id = job.getId();
...
%>
        ...
        <li>
          <div class="con lbcon2 lbcon3">
            <div class="btts"><div class="span1"><%=position %></div><div class="span2"><%=totalnumber %></div><div class="span3"><%=location %></div></div>
            <%=intro %>
          </div>
        </li>        
...
%>     	
	  		...
	  		<!-- ------------以下HTML分页区------------ -->
	  	<form name="form1"  action="Job-list.action">	
	  ...
	  <a class="btn1" href="Job-list.action?countNumber=7&pageNo=<%=pageNo-1%>" >上一页</a>
	  <a class="btn1 btn2" href="Job-list.action?countNumber=7&pageNo=<%=pageNo+1%>" target="_blank">下一页</a>
