<%@ page language="java" import="java.util.*,net.fckeditor.*, java.net.*"	pageEncoding="UTF-8"%>
<%@ page
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			var flag = false;
			$(function(){
				$.formValidator.initConfig({formID:"url-add",status:"sumbitingWithAjax", onError:function(msg){alert(msg);},onSuccess:function(){flag = true; return true;}});
				...
				$("#title").formValidator({onShow:"请输入标题",onFocus:"必填项",onCorrect:"<font color='green'>输入正确</font>"}).inputValidator({min:4,max:200,onError:"<font color='red'>最少4位</font>"});
				...
				$("#urladdress").formValidator({onShow:"请输入链接",onFocus:"必填项",onCorrect:"<font color='green'>输入正确</font>"}).inputValidator({min:4,max:1000,onError:"<font color='red'>最少4位</font>"});
			});
			function znzn_urlAdd() {
				$("#url-add").bind('submit',function(){
					//alert(flag); 
					if(flag==false){
						return false;
					}else{
						...
						var descr = $("#descr").val();
						var tags = $("#tags").val();
						$.ajax({url:'ajax/url-add',type : 'POST', data:{'idandname' : idandname, 'title' : title, 'urladdress' : urladdress, 'tags' : tags,'origin' : origin, 'descr' : descr}, success:function(data) {alert('add success!');TB_remove();}
								}); 
						flag = false;
						return false;
					}
	...
	String intCategoryName = request.getParameter("categoryname");
	String categoryName = CommonUtil.intString2CharArray2chString(intCategoryName);
	...
	%>
		<div class="znzn_urladd">
			<div class="znzn_title">
				...
				<form action="" method="post" name="url-add" id="url-add" onsubmit="znzn_urlAdd();">
